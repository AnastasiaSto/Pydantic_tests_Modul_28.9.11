...
def test_users_get_response():
    response = [
        {"id": 123, "first_name": "Mark", "last_name": "Twen"},
        {"id": 456, "first_name": "Jane", "last_name": "Air"}
    ]
...
def test_users_get_success():
    ...
    assert users[0].first_name == "Mark"
    assert users[0].last_name == "Twen"
...
def test_users_get_one_user():
    response = [{"id": 123, "first_name": "Mark", "last_name": "Twen"}]
    users = [User(**user) for user in response]
    assert len(users) == 1
    assert users[0].id == 123
    assert users[0].first_name == "Mark"
